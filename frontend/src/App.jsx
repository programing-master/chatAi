...
function App () {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const newMessage = { message: text, timestamp: Date.now() }
      setHistory(prevHistory => [...prevHistory, newMessage])
  
      const cleanedResponse = cleanResponse(text)
      setResponse(cleanedResponse)
      
      // Esperar un poco antes de hacer el scroll
      setTimeout(() => {
        // Hacer el scroll hasta abajo con animación suave
        window.scrollTo({
      ...
      setResponse(
        'Ocurrió un error al enviar el mensaje ,Actualiza la página o verifica tu conexión a internet'
      )
    }
  }
  ...
  return (
    <>
      ...
      <div className='w-full flex flex-col gap-1 min-h-[100vh] items-center  '>
        ...
        <div className='mt-20 flex flex-col min-h-screen md:flex-row w-full gap-2 p-1 bg-gradient-to-b from-black  to-dark-onPrimary'>
          {/*main*/}
          <div className='chat-container flexflex-col gap-20 w-full'>
            ...
            <div className='chat-container w-full  items-end flex flex-col gap-2 justify-end '>
              <h2 className="w-full text-center mt-2">History</h2>
              {history &&
                history.map((item, index) => (
                  <div
                    key={item.timestamp}
                    className='border  bg-light-onPrimaryContainer text-gray p-4 rounded border-dark-onPrimary  gap-4 items-start justify-between min-h-20 flex'
                  >
